...
####### Added 1 this to read XML-Roberta model #########
...
class FrameAxis:
    def __init__(self, mfd=None, model_name='xlm-roberta-base'):
        self.tokenizer = XLMRobertaTokenizer.from_pretrained(model_name)
        self.model = XLMRobertaModel.from_pretrained(model_name)
        # self.vocab = self.tokenizer.get_vocab().keys() # for xlm-roberta-base  
        ...
        if mfd == "emfd":
            ...
            #### self.vocab for computing microframes is the our self-defined moral foundation dictionary ####
            self.vocab = words_df['word'].tolist() 
    ...
    ##### added 2 this to get the embedding of a word ##### 
        ...
        for word in words:
            ...
            for mf in self.axes.keys():
                if word in self.vocab:
                    row_dict[mf] = self.cos_sim(self.get_embedding(word), self.axes[mf])
                else:
                    row_dict[mf] = np.nan
    ...
    def cos_sim(self, a, b):
        # Flatten the vectors to ensure they are 1-dimensional
        a = a.flatten()
        b = b.flatten()
    ...
    def framing_scores(self, doc_tokens, mf, B_T=None):
        ...
        for token in doc_tokens_set:
            sum_freq += freq[token]
            bias_score += (freq[token] * self.cos_sim(self.get_embedding(token), self.axes[mf]))
            if B_T is not None:
                intensity_score += (freq[token] * (self.cos_sim(self.get_embedding(token), self.axes[mf]) - B_T) ** 2)
    ...
    def framing_scores_tfidf(self, doc_tokens, mf, B_T=None, doc_idx=None):
        ...
        for token in doc_tokens_set:
            ...
            if token not in self.cos_sim_dict[mf]:
                self.cos_sim_dict[mf][token] = self.cos_sim(self.get_embedding(token), self.axes[mf])
    ...
    def doc_scores(self, docs, baseline_docs, tfidf=False):
        ...
        # this code will not be executed because baseline_doc is empty
        if baseline_docs:
        ...
        for mf in self.axes.keys():
            ...
            for idx in tqdm(range(len(docs)), desc="Processing documents"):
                if idx % 100 == 0:
                    print(f'Current doc_idx: {idx}/ Total: {len(docs)}')
                doc = docs[idx]
                doc_tokens = self.tokenizer.tokenize(doc)
                # doc_tokens = [x for x in doc_tokens_multilingual if x in self.vocab]
                if len(doc_tokens) == 0:
...
##### we only need intensity scores, don't need bias scores
    def get_fa_scores(self, 
        ...
        # i don't want to drop NA value rows, keeping them
        ...
        print('After adding bias and intensity scores, the shape:', fa_scores.shape)
