...
# download the llama3.1-8b pretrained model
...
# info_example = """以下是预期输入和输出的示例：
# - {user: '为了厘清1978年以后的思想混乱状况，中共中央认为，我们必须坚持社会主义道路；必须坚持无产阶级专政；必须坚持共产党的领导；必须坚持马列主义、毛泽东思想。这一定调为全党全国指明了方向。此次事件再次证明，一小撮卖国集团绑架民意，危害地区稳定。我们抛头颅洒热血也要和卖国贼斗争到底，卖国集团不除，国家永无宁日。对待卖国汉奸，我们应该只有炮弹没有糖衣。'}
# - {gpt: 'authority, loyalty'}

# - {user: '今年四月有上千名储户合计超过12亿的存款无法取出，这些储户有一个共同点，他们都是在相同的5家银行办理了储蓄业务。新闻爆出后，自然也让相关的其他储户心里亮起了警灯，结果所有人都无法正常取款，很多储户不得已便去银行门口聚集反抗，以此来维护他们的权益。最后，经统计有397亿存款不翼而飞，这些储户仍在用各种方式上访、投诉，以追回自己的存款。'}
# - {gpt: 'authority, fairness'}

# - {user: '妈祖，中国古代神话中的海神，又称天后、天上圣母等等 ，是历代船工、海员、旅客、商人和渔民共同信奉的神祇，祈求保佑顺风和安全。20世纪80年代，联合国有关机构授予妈祖“和平女神”称号。目前，全世界共有上万座从湄洲祖庙分灵的妈祖庙，有3亿多人信仰妈祖。各妈祖庙都会举办各式各样的纪念活动。'}
# - {gpt: : 'sanctity'}
# """
...
# {id: 'copy the id of the document', value: 'moral foundation values', rationale: 'brief rationale for the value'}
# """
...
# english labelled training set usage example: https://github.com/joshnguyen99/moral_axes/tree/bce1844fd6c73c6c839aecb1e4ad9172b163e3c7/scripts/mfd
# the goal is to finetune a multi-class classifier to predict the moral foundation values, instead of 5 binary classifiers, because the model will learn better difference between classes
### Label explanation:
    # -1: the sentence has not been seen by anyone assigned this foundation
    #  0: the sentence has been seen at least once with this foundation but was not highlighted
    # +1: the sentence has been seen at least once with this foundation,and was highlighted at least once
dataset_ft = pd.read_csv('/data/scro4316/thesis/paper3/e_tools_lm/data/mf_corpora_merged.csv', index_col=0)
dataset_ft = dataset_ft[['sentence', 'tokens','authority_label', 'care_label',  'fairness_label', 'loyalty_label',  'sanctity_label', 'none_label']]
dataset_ft.columns = ['text', 'tokens','authority', 'care',  'fairness', 'loyalty',  'sanctity', 'none']

# create a new column, which is a list of all the labels where value is 1, and separate by comma
dataset_ft['labels'] = dataset_ft[['authority', 'care',  'fairness', 'loyalty',  'sanctity', 'none']].apply(lambda x: ','.join(x.index[x == 1]), axis=1)

# remove the rows where the labels are empty
dataset_ft = dataset_ft[['text', 'labels']]
...
### maximum tokens number = 4028
### system_message tokens number = 1278
...
# Clean data by removing newlines
def remove_newlines(text):
    if isinstance(text, str):
        return text.replace('\n', '')
    return text

# convert to hugging face dataset format - first convert to dataframe, then to dataset, 
# somehow the dictionary format function Data.from_dict() is not working
prompt_df = pd.DataFrame(prompt_list, columns=['conversations'])
dataset = Dataset.from_pandas(prompt_df)
dataset = dataset.map(formatting_prompts_func, batched = True,)
## note1: checked - the hf_dataset is the same as format as the tutorial sentiment dasaset
## note2: the sentiment datast demo: https://huggingface.co/seandearnaley/neuraldaredevil-8b-abliterated-sentiment-analysis-june-05-2024-1-epoch
...
trainer = SFTTrainer(
    ...
    args = TrainingArguments(
        ...
        warmup_steps = 5,
        num_train_epochs=1, # this is for the full training run (for quick test, run the training for 60 steps)
        # max_steps = 20, # for quick test, for all run, set it as none
        learning_rate = 2e-4,
...
# for L40S, it takes 5 hours ish to finish 
# for A100, ?
...
for i in range(0, len(inference_list)):
    ...
    csv_file_path = "/result_llama_ft_en_prompt_ch.csv"
    ...
    print(f"{i} records have been successfully processed and saved.")


# Local saving
model_save_path = "ft_prompt_ch_model"
...
#     from unsloth import FastLanguageModel
#     model, tokenizer = FastLanguageModel.from_pretrained(
#         model_name = "/data/scro4316/thesis/paper3/llama3.1/finetune_models/en_ft_model", # YOUR MODEL YOU USED FOR TRAINING
#         max_seq_length = max_seq_length,
#         dtype = dtype,
#         load_in_4bit = load_in_4bit,
#     )
...
# inputs = generate_input(inference_list[200])
